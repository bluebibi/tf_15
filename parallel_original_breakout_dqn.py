...
def play_func(exp_queue, mainDQN):
    ...
    while epoch <= 200:
        ...
        while not d:
            ...
            # TODO
            exp_queue.put((history, action, reward, ter))
...
def main():
    ...
    with tf.Session() as sess:
        ...
        exp_queue = mp.Queue(maxsize=TRAIN_FREQUENCY * 2)

        play_proc = mp.Process(target=play_func, args=(exp_queue, None))
        play_proc.start()
        ...
        while play_proc.is_alive():
            ...
            if frame_idx % TARGET_UPDATE < TRAIN_FREQUENCY:
                copy_ops = get_copy_var_ops(dest_scope_name="target", src_scope_name="main")
                sess.run(copy_ops)
